...
void heap_pop(Heap* pq){
  void *aux_data = pq->heapArray[0].data;
  int aux_priority = pq->heapArray[0].priority;
  ...
  while (i != pq->size-1) {
    if (pq->heapArray[i].priority < pq->heapArray[2*i+2].priority) {
        void *aux_data = pq->heapArray[2*i+2].data;
        int aux_priority = pq->heapArray[2*i+2].priority;
        pq->heapArray[2*i+2].data = pq->heapArray[i].data;
        pq->heapArray[2*i+2].priority = pq->heapArray[i].priority;
        pq->heapArray[i].data = aux_data;
        pq->heapArray[i].priority = aux_priority;
    }
    i = 2*i+2;
